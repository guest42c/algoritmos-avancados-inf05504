...
echo "m:"
read m
echo "inserts lookups table_size elapsed_time_inserts elapsed_time_lookups" > "experiment-cuco-$n-$m-all"
...
do
  ...
  do
    echo "Execution $j"
    ./gen $i $m > "test-$i-$m.gr"
    ./hash_cuco < "test-$i-$m.gr" 2>> "run.tmp"
    rm "test-$i-$m.gr"
  done
  ...
  cat run.tmp >> "experiment-cuco-$n-$m-all"
  rm run.tmp
  echo $occupancy $min_inserts $inserts $max_inserts >> "experiment-cuco-$n-$m-inserts"
  echo $occupancy $min_lookups $lookups $max_lookups >> "experiment-cuco-$n-$m-lookups"
done   
